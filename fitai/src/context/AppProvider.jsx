...
export default function AppProvider({ children }) {
  ...
  const update = useCallback((patch) => {
    setState((prev) => {
      ...
      localStorage.setItem(
        "befit-ui",
        JSON.stringify({ darkMode: next.darkMode })
      );
  ...
  const loadUserData = async () => {
    ...
    try {
      const [profileData, statsData, mealsData, workoutsData] =
        await Promise.all([
      ...
      setState((prev) => ({
        ...
        profile: profileData?.profile
          ? { ...prev.profile, ...profileData.profile }
          : prev.profile,

        goals: profileData?.goals
          ? { ...prev.goals, ...profileData.goals }
          : prev.goals,

        stats: statsData
          ? {
              ...prev.stats,
              calories: statsData.calories_consumed || 0,
              protein: statsData.protein_consumed || 0,
              water: statsData.water_consumed || 0,
            }
          : prev.stats,
  ...
  const updateStat = useCallback(async (key, val) => {
    ...
    update({
      stats: { ...state.stats, [key]: val }
    });

    try {
      await statsAPI.update({
        calories_consumed:
          key === "calories" ? val : state.stats.calories,
        protein_consumed:
          key === "protein" ? val : state.stats.protein,
        water_consumed:
          key === "water" ? val : state.stats.water,
      });
    } catch {}
  ...
  const addMeal = useCallback(async (mealType, food) => {
    ...
    const newLog = {
      ...state.mealLog,
      [mealType]: [...(state.mealLog[mealType] || []), food],
    };
    ...
    update({
      ...
      stats: {
        ...state.stats,
        calories: state.stats.calories + food.calories,
        protein: state.stats.protein + food.protein,
      },
    });
  ...
  const removeMeal = useCallback(async (mealType, idx) => {
    ...
    const newLog = {
      ...state.mealLog,
      [mealType]: state.mealLog[mealType].filter((_, i) => i !== idx),
    };
    ...
    update({
      ...
      stats: {
        ...state.stats,
        calories: Math.max(0, state.stats.calories - food.calories),
        protein: Math.max(0, state.stats.protein - food.protein),
      },
    });

    try {
      if (food.id) {
        await mealsAPI.remove(food.id);
      }
    } catch {}
  ...
  const toggleWorkout = useCallback(async (group, exerciseId) => {
    ...
    const current = state.workouts[group]?.find(
      (w) => w.id === exerciseId
    );
    ...
    const updated = groupExercises.find((w) => w.id === exerciseId)
      ? groupExercises.map((w) =>
          w.id === exerciseId ? { ...w, done } : w
        )
      : [...groupExercises, { id: exerciseId, done }];

    update({
      workouts: { ...state.workouts, [group]: updated },
    });

    try {
      await workoutsAPI.toggle({
        muscle_group: group,
