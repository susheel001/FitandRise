...
import { mdiWeightLifter, mdiFire, mdiDumbbell, mdiFood, mdiWater } from '@mdi/js';

export default function Progress() {
  const { state } = useApp();
  const { darkMode: dm, stats, goals, profile } = state;
  const [weekly, setWeekly] = useState([]);
  const [activeChart, setActiveChart] = useState('calories'); // calories | water
  ...
  const summaryCards = [
    ...
    { icon: mdiWater,        label: 'Water Today',    value: `${stats.water} gl`,      goal: `Goal: ${goals.water} glasses`,color: '#06b6d4', bg: dm ? '#164e63' : '#ecfeff' },
  ...
  const tabClass = (key) =>
    `px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${
      activeChart === key
        ? 'text-white'
        : dm ? 'text-gray-400 hover:text-white' : 'text-gray-400 hover:text-gray-700'
    }`;

  const tabStyle = (key) => activeChart === key
    ? { background: key === 'calories' ? 'linear-gradient(135deg,#3b82f6,#22c55e)' : 'linear-gradient(135deg,#06b6d4,#3b82f6)' }
    : {};
  ...
  return (
    <div className="space-y-4">
      ...
      <div className={card}>
        ...
        <div className="space-y-3">
          {[
            ...
            { label: 'Water',    val: stats.water,    max: goals.water,    color: '#06b6d4', unit: 'glasses' },
          ].map(g => (
            <div key={g.label}>
              ...
              <div className={`w-full h-2.5 rounded-full overflow-hidden ${dm ? 'bg-gray-600' : 'bg-gray-100'}`}>
                <div className="h-2.5 rounded-full transition-all duration-700"
                  style={{ width: `${Math.min((g.val / g.max) * 100, 100)}%`, backgroundColor: g.color }} />
              </div>
            </div>
          ))}
        </div>
      ...
      {/* Weekly charts with tabs */}
      {weekly.length > 0 ? (
        <div className={card}>
          <div className="flex items-center justify-between mb-4">
            <p className={`font-bold text-sm sm:text-base ${textMain}`}>Weekly Trends</p>
            <div className={`flex gap-1 p-1 rounded-xl ${dm ? 'bg-gray-700' : 'bg-gray-100'}`}>
              <button className={tabClass('calories')} style={tabStyle('calories')} onClick={() => setActiveChart('calories')}>
                Calories & Protein
              </button>
              <button className={tabClass('water')} style={tabStyle('water')} onClick={() => setActiveChart('water')}>
                Water
              </button>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={weekly}>
                ...
                <Line type="monotone" dataKey="water" stroke="#06b6d4" strokeWidth={2.5}
                  dot={{ fill: '#06b6d4', r: 4 }} name="Water (glasses)" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className={`${card} text-center py-10`}>
          <p className={`text-sm ${textMuted}`}>Start logging your meals and water to see weekly charts here!</p>
        </div>
      )}
